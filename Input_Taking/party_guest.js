...
function runProgram(input){
    ...
    input = input.split("\n");
    var pam = +input[0];
    var pam_list = input[1].split(" ").map(Number);
    var jim = +input[2];
    var jim_list = input[3].split(" ").map(Number);
    
    partyGuest(pam,pam_list,jim,jim_list);
    
}

function partyGuest(p,pl,j,jl){
    
    var obj = {};
    
    
    for(var num = 0; num < p; num++)
    {
        obj[pl[num]] = 1;
    }
    var answer = "Yes";
    for(var num2 = 0; num2 < j; num2++)
    {
        if (obj[jl[num2]] == undefined)
        {
            answer = "No";
        }
